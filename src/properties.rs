...
/// level number (e.g., "0").
pub fn num_files_at_level(level:usize) -> PropertyName { unsafe { level_property("num-files-at-level", level) } }
...
/// based.
pub const ESTIMATE_PENDING_COMPACTION_BYTES:&PropName = property!("estimate-pending-compaction-bytes");
