...
///
/// The default thread mode for [`OptimisticTransactionDB`] is
/// [`SingleThreaded`] if feature `multi-threaded-cf` is not enabled.
///
...
///
/// ```
/// use rocksdb::{DB, OptimisticTransactionDB, Options, SingleThreaded};
...
pub type OptimisticTransactionDB<T = crate::SingleThreaded> = DBCommon<T, OptimisticTransactionDBInner>;
...
impl<T:ThreadMode> OptimisticTransactionDB<T> {
	...
	/// `Options`. *NOTE*: `default` column family will be opened with the
