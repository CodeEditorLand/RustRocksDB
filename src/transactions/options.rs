...
impl TransactionOptions {
	...
	/// have not been modified since the time each key was first written (or
	/// fetched via `get_for_update`).
	///
	...
	/// Specifies the wait timeout in milliseconds when a transaction attempts
	/// to lock a key.
	///
	/// If 0, no waiting is done if a lock cannot instantly be acquired.
	/// If negative, transaction lock timeout in `TransactionDBOptions` will be
	/// used.
	///
	/// Default: -1.
	...
	///
	/// If non-negative, transactions that last longer than this many
	/// milliseconds will fail to commit. If not set, a forgotten transaction
	/// that is never committed, rolled back, or deleted will never relinquish
	/// any locks it holds.  This could prevent keys from being by other
	/// writers.
	///
	/// Default: -1.
	...
	/// Specifies the maximum number of bytes used for the write batch. 0 means
	/// no limit.
	///
	/// Default: 0.
...
impl TransactionDBOptions {
	...
	///
	/// If 0, no waiting is done if a lock cannot instantly be acquired.
	/// If negative, there is no timeout and will block indefinitely when
	/// acquiring a lock.
	///
	...
	/// cannot deadlock with other DB writes, they can deadlock with a
	/// transaction. A negative timeout should only be used if all transactions
	...
	///
	/// If 0, no waiting is done if a lock cannot instantly be acquired.
	/// If negative, there is no timeout.  Not using a timeout is not
	/// recommended as it can lead to deadlocks.  Currently, there is no
	...
	/// transaction `writes` (or `get_for_update`) will return an error.
