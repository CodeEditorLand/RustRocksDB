...
/// RocksDB Transaction.
///
/// To use transactions, you must first create a [`TransactionDB`] or
...
impl<DB> Transaction<'_, DB> {
	...
	///
	/// If this transaction was created by an [`OptimisticTransactionDB`], an
	/// error of the [`Busy`] kind may be returned if the transaction
	/// could not guarantee that there are no write conflicts.
	/// An error of the [`TryAgain`] kind may be returned if the memtable
	/// history size is not large enough (see
	/// [`Options::set_max_write_buffer_size_to_maintain`]).
	///
	...
	pub fn set_name(&self, name: &[u8]) -> Result<(), Error> {
		...
		let len = name.len();
		unsafe {
			ffi_try!(ffi::rocksdb_transaction_set_name(self.inner, ptr as _, len as _));
		}
	...
	/// Returns snapshot associated with transaction if snapshot was enabled in
	/// [`TransactionOptions`]. Otherwise, returns a snapshot with `nullptr`
	...
	/// [`rollback_to_savepoint`]. May be called multiple times to set multiple
	/// save points.
	///
	...
	/// Undo all operations in this transaction since the most recent call to
	/// [`set_savepoint`] and removes the most recent [`set_savepoint`].
	///
	...
	/// Get the key in the given column family and ensure that this transaction
	/// will only be able to be committed if this key is not written outside
	/// this transaction after it has first been read (or after the snapshot if
	/// a snapshot is set in this transaction).
	///
	/// See [`get_for_update_cf_opt`] for details.
	///
	/// [`get_for_update_cf_opt`]: Self::get_for_update_cf_opt
	...
	/// kind if the most recent write to the queried key in this batch is a
	/// Merge.
	///
	...
	/// `get_for_update_opt` can cause [`commit`] to fail. Otherwise, it could
	/// return any error that could be returned by `[DB::get]`.
	///
	/// [`Busy`]: crate::ErrorKind::Busy
	...
	/// Put the key value in default column family and do conflict checking on
	/// the key.
	///
	...
	/// Put the key value in the given column family and do conflict checking on
	/// the key.
	///
	/// If this transaction was created by a [`TransactionDB`], it can return
	/// error of kind:
	...
	/// Merge value with existing value of key, and also do conflict checking on
	/// the key.
	///
	...
	/// Delete the key value in the given column family and do conflict
	/// checking.
	///
	/// If this transaction was created by a [`TransactionDB`], it can return
	/// error of kind:
