...
/// [`MultiThreaded`] mode. Clone (and Copy) is derived to behave like
/// `&ColumnFamily` (this is used for single-threaded mode). `Clone`/`Copy` is
/// safe because this lifetime is bound to DB like iterators/snapshots. On top
...
/// [`ColumnFamily`] depending on the `multi-threaded-cf` crate feature.
...
// implementation detail, considering use of std::mem::transmute.
