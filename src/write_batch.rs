...
/// A type alias to keep compatibility. See [`WriteBatchWithTransaction`] for
/// details
...
unsafe extern fn writebatch_put_callback(state:*mut c_void, k:*const c_char, klen:usize, v:*const c_char, vlen:usize) {
	...
	let key = unsafe { slice::from_raw_parts(k as *const u8, klen) };
	let value = unsafe { slice::from_raw_parts(v as *const u8, vlen) };
...
impl<const TRANSACTION: bool> WriteBatchWithTransaction<TRANSACTION> {
	...
	/// Return a reference to a byte array which represents a serialized version
	/// of the batch.
	pub fn data(&self) -> &[u8] {
		unsafe {
			let mut batch_size:size_t = 0;
	...
	pub fn delete<K:AsRef<[u8]>>(&mut self, key:K) {
		...
		unsafe {
			ffi::rocksdb_writebatch_delete(self.inner, key.as_ptr() as *const c_char, key.len() as size_t);
		}
	}
