...
/// later All keys in files generated by SstFileWriter will have sequence number
/// = 0.
...
impl<'a> SstFileWriter<'a> {
	...
	pub fn create(opts:&'a Options) -> Self {
		...
		Self { inner:writer, phantom:PhantomData }
	}
	...
	fn open_raw(&'a self, cpath:&CString) -> Result<(), Error> {
		unsafe {
			ffi_try!(ffi::rocksdb_sstfilewriter_open(self.inner, cpath.as_ptr() as *const _));
	...
	/// Adds a deletion key to currently opened file
	/// REQUIRES: key is after any previously added key according to comparator.
	pub fn delete_with_ts<K:AsRef<[u8]>, S:AsRef<[u8]>>(&mut self, key:K, ts:S) -> Result<(), Error> {
		let key = key.as_ref();
		let ts = ts.as_ref();
		unsafe {
