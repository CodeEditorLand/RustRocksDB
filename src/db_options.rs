...
use crate::{
	...
	comparator::{ComparatorCallback, ComparatorWithTsCallback, CompareFn, CompareTsFn, CompareWithoutTsFn},
	db::DBAccess,
	env::Env,
	ffi,
	ffi_util::{CStrLike, from_cstr, to_cpath},
	merge_operator::{self, MergeFn, MergeOperatorCallback, full_merge_callback, partial_merge_callback},
	slice_transform::SliceTransform,
	statistics::{Histogram, HistogramData, StatsLevel, Ticker},
};
...
impl WriteBufferManager {
	...
	///
	/// 1- Limit the total memtable usage across multiple column families and
	/// DBs under a threshold. 2- Cost the memtable memory usage to block cache
	/// so that memory of RocksDB can be capped by the single limit.
	/// The usage of a write buffer manager is similar to rate_limiter and
	...
	///
	/// a flush will be triggered on one column family of the DB you are
	/// inserting to,
	///
	...
	/// triggered only if the mutable memtable size also exceeds 50% of the
	/// limit. Both checks are needed because if already more than half memory
	/// is being flushed, triggering more flush may not help.
	///
	/// The total memory is counted as total memory allocated in the arena, even
	/// if some of that may not yet be used by memtable.
	///
	...
	/// Users can set up RocksDB to cost memory used by memtables to block
	/// cache. This can happen no matter whether you enable memtable memory
	/// limit or not. This option is added to manage memory (memtables + block
	/// cache) under a single limit.
	///
	/// buffer_size: the memory limit in bytes.
	/// allow_stall: If set true, it will enable stalling of all writers when
	/// memory usage exceeds buffer_size (soft limit).             It will wait
...
impl Cache {
	...
	///
	/// * If the estimate is substantially too high (e.g., 25% higher), the
	///   cache may have to evict entries to prevent load factors that would
	///   dramatically affect lookup times.
	/// * If the estimate is substantially too low (e.g., less than half), then
	///   meta data space overhead is substantially higher.
	///
...
/// and most importantly, measure performance under realistic workloads with
/// realistic hardware.
///
/// # Examples
///
...
///
/// ```
/// use rocksdb::{DB, FlushOptions, Options};
///
/// let tempdir = tempfile::Builder::new()
...
///
/// ```
/// use rocksdb::{DB, IngestExternalFileOptions, Options, SstFileWriter};
///
...
// the inner FFI pointer. In most cases, however, this pointer is Send-safe
// because it is never aliased and rocksdb internally does not rely on
// thread-local information for its user-exposed types.
...
// mutability, and their use within the rocksdb library is generally behind a
// const reference
...
impl BlockBasedOptions {
	...
	/// kTwoLevelIndexSearch is used and to filters when partition_filters is
	/// used. Note: Since in the current implementation the filters and index
	/// partitions are aligned, an index/filter block is created when either
	/// index or filter block size reaches the specified limit.
	///
	...
	/// probably set the `format_version` to >= 4 as it would reduce the index
	/// size.
	///
	/// Default: 1.
	...
	///  `DataBlockIndexType::BinarySearch` to use binary search within the data
	/// block.  `DataBlockIndexType::BinaryAndHash` to use the data block hash
	/// index in combination with  the normal binary search.
	///
	/// The hash table utilization ratio is adjustable using
	/// [`set_data_block_hash_ratio`](#method.set_data_block_hash_ratio), which
	/// is valid only when using `DataBlockIndexType::BinaryAndHash`.
	///
	...
	///
	/// The smaller the utilization ratio, the less hash collisions happen, and
	/// so reduce the risk for a point lookup to fall back to binary search due
	/// to the collisions. A small ratio means faster lookup at the price of
	/// more space overhead.
	///
	/// Default: 0.75
...
impl Options {
	...
	///
	/// *IMPORTANT*:
	/// ROCKSDB DOES NOT STORE cf ttl in the options file. If you have set it
	...
	/// cores. You almost definitely want to call this function if your system
	...
	///
	/// Default values for some parameters in `Options` are not optimized for
	/// heavy workloads and big datasets, which means you might observe write
	/// stalls under some conditions.
	///
	/// This can be used as one of the starting points for tuning RocksDB
	/// options in such cases.
	///
	/// Internally, it sets `write_buffer_size`,
	/// `min_write_buffer_number_to_merge`, `max_write_buffer_number`,
	/// `level0_file_num_compaction_trigger`, `target_file_size_base`,
	/// `max_bytes_for_level_base`, so it can override if those parameters were
	/// set before.
	///
	...
	/// Specifies whether an error should be raised if the database already
	/// exists.
	///
	/// Default: false
	...
	/// by determining where to place a file are based on best-effort
	/// estimation, which means there is a chance that the actual size under
	/// the directory is slightly more than target size under some workloads.
	...
	/// [`set_bottommost_compression_options`](#method.
	/// set_bottommost_compression_options) or
	/// [`set_bottommost_zstd_max_train_bytes`](#method.
	/// set_bottommost_zstd_max_train_bytes) method with `enabled` argument set
	/// to `true`.
	///
	/// # Examples
	///
	...
	/// the dictionary.
	///
	/// When compression dictionary is disabled, we compress and write each
	/// block before buffering data for the next one. When compression
	/// dictionary is enabled, we buffer all SST file data in-memory so we can
	/// sample it, as data can only be compressed and written after the
	/// dictionary has been finalized. So users of this feature may see
	...
	/// of all settings is the same as in
	/// [`set_compression_options`](#method.set_compression_options) method but
	...
	/// With this feature turned on, RocksDB will automatically adjust max bytes
	/// for each level. The goal of this feature is to have lower bound on size
	/// amplification.
	///
	/// Default: false.
	...
	///
	/// Leveled: files older than `periodic_compaction_seconds` will be picked
	/// up for compaction and will be re-written to the same level as they were
	/// before.
	///
	/// FIFO: not supported. Setting this option has no effect for FIFO
	/// compaction.
	///
	/// Universal: when there are files older than
	/// `periodic_compaction_seconds`, rocksdb will try to do as large a
	/// compaction as possible including the last level. Such compaction is
	/// only skipped if only last level is to be compacted and no file in last
	/// level is older than `periodic_compaction_seconds`. See more in
	...
	///
	/// One main use of the feature is to make sure a file goes through
	/// compaction filters periodically. Users can also use the feature to
	/// clear up SST files using old format.
	///
	/// A file's age is computed by looking at file_creation_time or
	/// creation_time table properties in order, if they have valid non-zero
	/// values; if not, the age is based on the file's last modified time
	...
	/// Sets a compaction filter used to determine if entries should be kept,
	/// changed, or removed during compaction.
	///
	/// An example use case is to remove entries with an expired TTL.
	///
	/// If you take a snapshot of the database, only values written since the
	/// last snapshot will be passed through the compaction filter.
	///
	...
	/// an application to modify/delete a key-value during background
	/// compaction.
	///
	/// A new filter will be created on each compaction run.  If multithreaded
	/// compaction is being used, each created CompactionFilter will only be
	...
	/// Sets the comparator that are timestamp-aware, used to define the order
	/// of keys in the table, taking timestamp into consideration.
	...
	/// It's recommended to manually call CompactRange(NULL, NULL) before
	/// reading from the database, because otherwise the read can be very slow.
	...
	/// Sets the number of open files that can be used by the DB. You may need
	/// to increase this if your database has a large working set. Value `-1`
	/// means files opened are always kept open. You can estimate number of
	/// files based on target_file_size_base and target_file_size_multiplier
	/// for level-based compaction. For universal-style compaction, you can
	/// usually set it to `-1`.
	///
	/// Default: `-1`
	///
	...
	/// Specifies whether an iteration->Next() sequentially skips over keys with
	/// the same user-key or not.
	///
	...
	/// all write buffers are flushed to L0 as individual files and this
	/// increases read amplification because a get request has to check in all
	...
	/// means that level-0 compaction will not be triggered by number of files
	/// at all.
	///
	/// Default: `4`
	///
	...
	/// Sets the compaction priority. When multiple files are picked for
	/// compaction from a level, this option determines which files to pick
	/// first.
	///
	...
	///
	/// ```
	/// use rocksdb::{CompactionPri, Options};
	///
	/// let mut opts = Options::default();
	...
	/// Sets the soft limit on number of level-0 files. We start slowing down
	/// writes at this point. A value < `0` means that no writing slowdown will
	...
	/// If the application cannot tolerate the relaxed guarantees, it can
	/// implement its own mechanisms to work around that and yet benefit from
	/// the higher throughput. Using TransactionDB with WRITE_PREPARED write
	/// policy and two_write_queues=true is one way to achieve immutable
	/// snapshots despite unordered_write.
	///
	/// By default, i.e., when it is false, rocksdb does not advance the
	/// sequence number for new snapshots unless all the writes with lower
	/// sequence numbers are already finished. This provides the immutability
	/// that we expect from snapshots. Moreover, since Iterator and MultiGet
	/// internally depend on snapshots, the snapshot immutability results into
	/// Iterator and MultiGet offering consistent-point-in-time view. If set to
	/// true, although Read-Your-Own-Write property is still provided, the
	/// snapshot immutability property is relaxed: the writes issued after the
	/// snapshot is obtained (with larger sequence numbers) will be still not
	/// visible to the reads from that snapshot, however, there still might be
	/// pending writes (with lower sequence number) that will change the state
	...
	/// Sets the maximum number of concurrent background memtable flush jobs,
	...
	/// Sets the maximum number of successive merge operations on a key in the
	/// memtable.
	///
	/// When a merge operation is added to the memtable and the maximum number
	/// of successive merges is reached, the value of the key will be
	/// calculated and inserted into the memtable instead of the merge
	/// operation. This will ensure that there are never more than
	...
	/// If true, then DB::Open() will not fetch and check sizes of all sst
	/// files. This may significantly speed up startup if there are many sst
	/// files, especially when using non-default Env with expensive
	/// GetFileSize(). We'll still check that all required sst files exist.
	...
	/// including copies of buffers that have already been flushed. This
	/// parameter only affects trimming of flushed buffers and does not affect
	/// flushing. This controls the maximum amount of write history that will
	/// be available in memory for conflict checking when Transactions are
	/// used. The actual size of write history (flushed Memtables) might be
	/// higher than this limit if further trimming will reduce write history
	/// total size below this limit. For example, if
	/// max_write_buffer_size_to_maintain is set to 64MB, and there are three
	/// flushed Memtables, with sizes of 32MB, 20MB, 20MB. Because trimming the
	/// next Memtable of size 20MB will reduce total memory usage to 52MB which
	/// is below the limit, RocksDB will stop trimming.
	///
	...
	/// to the head of the queue becomes write batch group leader and
	/// responsible for writing to WAL and memtable for the batch group.
	///
	...
	///
	/// ```
	/// use rocksdb::{MemtableFactory, Options};
	/// let mut opts = Options::default();
	/// let factory =
	/// 	MemtableFactory::HashSkipList { bucket_count:1_000_000, height:4, branching_factor:4 };
	///
	/// opts.set_allow_concurrent_memtable_write(false);
	...
	pub fn set_memtable_factory(&mut self, factory:MemtableFactory) {
		match factory {
			...
			MemtableFactory::HashSkipList { bucket_count, height, branching_factor } => unsafe {
				ffi::rocksdb_options_set_hash_skip_list_rep(self.inner, bucket_count, height, branching_factor);
			},
	...
	///
	/// ```
	/// use rocksdb::{CuckooTableOptions, Options};
	///
	/// let mut opts = Options::default();
	...
	/// Once write-ahead logs exceed this size, we will start forcing the flush
	/// of column families whose memtables are backed by the oldest live WAL
	/// file (i.e. the ones that are causing all the space amplification).
	///
	/// Default: `0`
	///
	/// # Examples
	...
	/// Enable/disable adaptive mutex, which spins in the user space before
	/// resorting to kernel.
	///
	...
	/// When a `prefix_extractor` is defined through `opts.set_prefix_extractor`
	/// this creates a prefix bloom filter for each memtable with the size of
	...
	///    archive.
	/// 2. If wal_ttl_seconds is 0 and wal_size_limit_mb is not 0, WAL files
	///    will be checked every 10 min and if total size is greater then
	///    wal_size_limit_mb, they will be deleted starting with the earliest
	///    until size_limit is met. All empty files will be deleted.
	/// 3. If wal_ttl_seconds is not 0 and wall_size_limit_mb is 0, then WAL
	///    files will be checked every wal_ttl_seconds / 2 and those that are
	///    older than wal_ttl_seconds will be deleted.
	...
	/// Sets the number of bytes to preallocate (via fallocate) the manifest
	/// files.
	///
	...
	/// Turning off this feature will improve DBOpen time especially in disk
	/// environment.
	///
	/// Default: false
	...
	/// If enabled, WAL is not flushed automatically after each write. Instead
	/// it relies on manual invocation of `DB::flush_wal()` to write the WAL
	/// buffer to its file.
	///
	/// Default: false
	///
	...
	/// overwriting the old data. The value indicates how many such files we
	/// will keep around at any point in time for later use. This is more
	/// efficient because the blocks are already allocated and fdatasync does
	/// not need to update the inode after each write.
	///
	/// Default: 0
	///
	/// # Examples
	...
	/// Sets the threshold at which all writes will be slowed down to at least
	/// delayed_write_rate if estimated bytes needed to be compaction exceed
	/// this threshold.
	///
	/// Default: 64GB
	...
	/// Sets the bytes threshold at which all writes are stopped if estimated
	...
	/// If true, then print malloc stats together with rocksdb.stats when
	/// printing to LOG.
	///
	/// Default: false
	...
	/// Enable whole key bloom filter in memtable. Note this will only take
	/// effect if memtable_prefix_bloom_size_ratio is not 0. Enabling whole key
	/// filtering can potentially reduce CPU usage for point-look-ups.
	///
	...
	/// If this is set to true RocksDB will actively relocate valid blobs from
	...
	/// Sets the threshold that the GC logic uses to determine which blob files
	/// should be considered “old.”
	///
	/// For example, the default value of 0.25 signals to RocksDB that blobs
	/// residing in the oldest 25% of blob files should be relocated by GC.
	/// This parameter can be tuned to adjust the trade-off between write
	...
	/// Sets the blob cache.
	///
	/// Using a dedicated object for blobs and using the same object for the
	/// block and blob caches are both supported. In the latter case, note that
	/// blobs are less valuable from a caching perspective than SST blocks, and
	/// some cache implementations have configuration options that can be used
	/// to prioritize items accordingly (see Cache::Priority and
	...
	/// operation (such as deleting obsolete files directly or deleting
	/// memtable) and will instead schedule a background job to do it.
	///
	...
	/// Currently, only syncing closed WALs are tracked. Calling
	/// `DB::SyncWAL()`, etc. or writing with `WriteOptions::sync=true` to sync
	/// the live WAL is not tracked for performance/efficiency reasons.
	///
	...
	pub fn get_track_and_verify_wals_in_manifest(&self) -> bool {
		let val_u8 = unsafe { ffi::rocksdb_options_get_track_and_verify_wals_in_manifest(self.inner) };
		val_u8 != 0
	}

	/// The DB unique ID can be saved in the DB manifest (preferred, this
	/// option) or an IDENTITY file (historical, deprecated), or both. If this
	/// option is set to false (old behavior), then `write_identity_file` must
	/// be set to true. The manifest is preferred because
	///
	...
	///    copied by BackupEngine), so is not reliable for the provenance of a
	///    DB.
	///
...
impl WriteOptions {
	...
	/// If true and if user is trying to write to column families that don't
	/// exist (they were dropped), ignore the write (don't return an error). If
...
impl Default for LruCacheOptions {
	fn default() -> Self {
		...
		assert!(!inner.is_null(), "Could not create RocksDB LRU cache options");
...
pub enum ReadTier {
	...
	BlockCache,
	/// Reads persisted data. When WAL is disabled, this option will skip data
	/// in memtable.
	Persisted,
...
impl ReadOptions {
	...
	/// before failing an iterator seek as incomplete. The default value of 0
	/// should be used to never fail a request as incomplete, even on skipping
	/// too many keys.
	///
	/// Default: 0
	...
	/// schedule a background job in the flush job queue and delete obsolete
	/// files in background.
	///
	/// Default: false
	...
	/// readers until they are naturally deleted during compaction. This
	...
	/// If auto_readahead_size is set to true, it will auto tune the
	/// readahead_size during scans internally.
	/// For this feature to be enabled, iterate_upper_bound must also be
	/// specified.
	///
	/// NOTE: - Recommended for forward Scans only.
	///       - If there is a backward scans, this option will be disabled
	///         internally and won't be enabled again if the forward scan is
	///         issued again.
	///
	/// Default: true
	...
	/// loaded by the iterator pinned in memory as long as the iterator is not
	/// deleted, If used when reading from tables created with
	...
	/// Timestamp of operation. Read should return the latest data visible to
	/// the specified timestamp. All timestamps of the same database must be of
	/// the same length and format. The user is responsible for providing a
	/// customized compare function via Comparator to order <key, timestamp>
	/// tuples. For iterator, iter_start_ts is the lower bound (older) and
	/// timestamp serves as the upper bound. Versions of the same record that
	/// fall in the timestamp range will be returned. If iter_start_ts is
...
pub enum BlockBasedIndexType {
	...
	/// a prefix extractor has been provided through
...
pub enum DataBlockIndexType {
	...
	/// Appends a compact hash table to the end of the data block for efficient
	/// indexing. Backwards compatible with databases created without this
	/// feature. Once turned on, existing data will be gradually converted to
	/// the hash index format.
	BinaryAndHash = 1,
}
...
impl FifoCompactOptions {
	...
	///
	/// Once the total sum of table files reaches this, we will delete the
	/// oldest table file
	///
	/// Default: 1GB
...
impl UniversalCompactOptions {
	...
	/// database. For example, a size amplification of 2% means that a database
	...
	/// well as  the total size of C1...Ct as total_C, the compaction output
	/// file will be compressed iff
	...
	/// Sets the algorithm used to stop picking files into a single compaction
	/// run.
	///
	/// Default: ::Total
...
impl CompactOptions {
	...
	/// only one will actually schedule it while the other threads will simply
	/// wait for the scheduled manual compaction to complete. If
...
impl WaitForCompactOptions {
	...
	/// when timeout == 0, WaitForCompact() will wait as long as there's
...
#[cfg(test)]
mod tests {
	use crate::{Cache, CompactionPri, MemtableFactory, Options, db_options::WriteBufferManager};
	...
	fn test_set_write_buffer_manager() {
		...
		let write_buffer_manager = WriteBufferManager::new_write_buffer_manager_with_cache(100, false, lrucache);
	...
	fn compaction_pri() {
		...
		let options = std::fs::read_dir(tmp.path())
			...
			.find_map(|x| {
				let x = x.ok()?;
				x.file_name().into_string().unwrap().contains("OPTIONS").then_some(x.path())
			})
