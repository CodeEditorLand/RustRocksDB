...
/// A type alias to keep compatibility. See [`DBRawIteratorWithThreadMode`] for
/// details
...
///
/// This iterator is different to the standard ``DBIteratorWithThreadMode`` as
/// it aims Into replicate the underlying iterator API within RocksDB itself.
/// This should give access to more performance and flexibility but departs from
/// the widely recognized Rust idioms.
///
/// ```
...
impl<'a, D:DBAccess> DBRawIteratorWithThreadMode<'a, D> {
	...
	/// invalidated and [`valid`](DBRawIteratorWithThreadMode::valid) will
	...
	/// Seeks to the specified key or the first key that lexicographically
	/// follows it.
	///
	/// This method will attempt to seek to the specified key. If that key does
	/// not exist, it will find and seek to the key that lexicographically
	/// follows it instead.
	///
	/// # Examples
	///
	...
	pub fn seek<K:AsRef<[u8]>>(&mut self, key:K) {
		...
		unsafe {
			ffi::rocksdb_iter_seek(self.inner.as_ptr(), key.as_ptr() as *const c_char, key.len() as size_t);
		}
	}
	...
	/// The difference with ``.seek()`` is that if the specified key do not
	/// exist, this method will seek to key that lexicographically precedes it
	/// instead.
	///
	/// # Examples
	///
	...
	pub fn seek_for_prev<K:AsRef<[u8]>>(&mut self, key:K) {
		...
		unsafe {
			ffi::rocksdb_iter_seek_for_prev(self.inner.as_ptr(), key.as_ptr() as *const c_char, key.len() as size_t);
		}
	}
	...
	pub fn item(&self) -> Option<(&[u8], &[u8])> {
		if self.valid() { Some((self.key_impl(), self.value_impl())) } else { None }
	}

	/// Returns a slice of the current key; assumes the iterator is valid.
	fn key_impl(&self) -> &[u8] {
		// Safety Note: This is safe as all methods that may invalidate the buffer
...
impl Iterator for DBWALIterator {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		// Note that WriteBatches live independently of the WAL iterator so this is safe
		// to do
		unsafe {
			ffi::rocksdb_wal_iter_next(self.inner);
		}
