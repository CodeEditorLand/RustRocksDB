...
use crate::{DBCommon, Error, ThreadMode, db::DBInner, env::Env, ffi, ffi_util::to_cpath};

/// Represents information of a backup including timestamp of the backup
/// and the size (please note that sum of all backups' sizes is bigger than the
/// actual size of the backup directory because some data is shared by multiple
...
// thread since the underlying RocksDB backup engine is thread-safe.
...
impl BackupEngine {
	...
	/// Get a list of all backups together with information on timestamp of the
	/// backup and the size (please note that sum of all backups' sizes is
	/// bigger than the actual size of the backup directory because some data
	/// is shared by multiple backups). Backups are identified by their
	/// always-increasing IDs.
	///
	/// You can perform this function safely, even with other BackupEngine
	/// performing backups on the same directory
...
impl BackupEngineOptions {
	...
	/// Sets the number of operations (such as file copies or file checksums)
	/// that `RocksDB` may perform in parallel when executing a backup or
	/// restore.
	///
	/// Default: 1
...
impl RestoreOptions {
	/// Sets `keep_log_files`. If true, restore won't overwrite the existing log
	/// files in wal_dir. It will also move all log files from archive
	/// directory to wal_dir. Use this option in combination with
