...
fn test_load_latest() {
	...
	let (_, cfs) = Options::load_latest(&n, Env::new().unwrap(), true, Cache::new_lru_cache(1024 * 8)).unwrap();
...
fn test_block_based_options() {
	...
	let n = DBPath::new(path);
	{
		...
		let mut rocksdb_log =
			fs::File::open(format!("{}/LOG", (&n).as_ref().to_str().unwrap())).expect("rocksdb creates a LOG file");
...
fn test_add_compact_on_deletion_collector_factory() {
	...
	assert!(
		settings.contains(
			"CompactOnDeletionCollector (Sliding window size = 5 Deletion trigger = 10 Deletion ratio = 0.5)"
		)
	);
}
...
fn test_set_track_and_verify_wals_in_manifest() {
	...
	// TODO: Check that the MANIFEST actually contains WalAddition/WalDeletion
	// records
	opts.create_if_missing(true);
	opts.set_track_and_verify_wals_in_manifest(true);
	let db = DB::open(&opts, &path).unwrap();
...
fn test_set_write_dbid_to_manifest() {
	...
	// verify the DBID is preserved across checkpoints. If set to false this is not
	// true
	opts.create_if_missing(true);
	opts.set_write_dbid_to_manifest(true);
