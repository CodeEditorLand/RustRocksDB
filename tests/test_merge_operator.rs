...
use rocksdb::{DB, DBCompactionStyle, MergeOperands, Options, merge_operator::MergeFn};
...
fn counting_merge_test() {
	...
	opts.set_merge_operator("sort operator", test_counting_full_merge, test_counting_partial_merge);
...
fn failed_merge_test() {
	...
	match res.and_then(|_e| db.get(b"key")) {
		...
		Err(e) => {
			...
			assert!(msg.contains("Merge operator failed"), "unexpected merge error message: {}", msg);
		},
	}
}
