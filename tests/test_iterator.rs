...
fn test_iterator() {
	let n = DBPath::new("_rust_rocksdb_iterator_test");
	{
		...
		assert_iter(db.iterator(IteratorMode::From(b"k2", Direction::Forward)), &expected2[1..]);
		assert_iter_reversed(db.iterator(IteratorMode::From(b"k2", Direction::Reverse)), &expected[..2]);
		assert_iter_reversed(db.iterator(IteratorMode::From(b"zz", Direction::Reverse)), &expected2);
...
fn test_full_iterator() {
	let path = DBPath::new("full_iterator_test");
	{
		...
		// A normal iterator won't work here since we're using a HashSkipList for our
		// memory table implementation (which buckets keys based on their prefix):
