...
fn iterator_test_tailing() {
	let path = DBPath::new("_rust_rocksdb_iteratortest_tailing");
	{
		...
		for (i, (k, v)) in tail_iter.map(Result::unwrap).enumerate() {
			assert_eq!((k.to_vec(), v.to_vec()), (data[i].0.to_vec(), data[i].1.to_vec()));
			tot += 1;
		}
...
fn set_option_test() {
	let path = DBPath::new("_rust_rocksdb_set_optionstest");
	{
		...
		assert!(db.set_options(&[("disable_auto_compactions", "true")]).is_ok());
		assert!(db.set_options(&[("disable_auto_compactions", "false")]).is_ok());
		// invalid names/values should result in an error
		assert!(db.set_options(&[("disable_auto_compactions", "INVALID_VALUE")]).is_err());
		assert!(db.set_options(&[("INVALID_NAME", "INVALID_VALUE")]).is_err());
		// option names/values must not contain NULLs
		assert!(db.set_options(&[("disable_auto_compactions", "true\0")]).is_err());
		assert!(db.set_options(&[("disable_auto_compactions\0", "true")]).is_err());
		// empty options are not allowed
		assert!(db.set_options(&[]).is_err());
		// multiple options can be set in a single API call
		let multiple_options = [("paranoid_file_checks", "true"), ("report_bg_io_stats", "true")];
...
fn set_option_cf_test() {
	let path = DBPath::new("_rust_rocksdb_set_options_cftest");
	{
		...
		// set an option to valid values
		assert!(db.set_options_cf(&cf, &[("disable_auto_compactions", "true")]).is_ok());
		assert!(db.set_options_cf(&cf, &[("disable_auto_compactions", "false")]).is_ok());
		// invalid names/values should result in an error
		assert!(
			db.set_options_cf(&cf, &[("disable_auto_compactions", "INVALID_VALUE")])
				.is_err()
		);
		assert!(db.set_options_cf(&cf, &[("INVALID_NAME", "INVALID_VALUE")]).is_err());
		// option names/values must not contain NULLs
		assert!(db.set_options_cf(&cf, &[("disable_auto_compactions", "true\0")]).is_err());
		assert!(db.set_options_cf(&cf, &[("disable_auto_compactions\0", "true")]).is_err());
		// empty options are not allowed
...
fn test_open_cf_descriptors_as_secondary() {
	...
	let secondary_path = DBPath::new("_rust_rocksdb_test_open_cf_descriptors_as_secondary_secondary");
	...
	let secondary_db =
		DB::open_cf_descriptors_as_secondary(&secondary_opts, &primary_path, &secondary_path, cfs).unwrap();
	let secondary_cf1 = secondary_db.cf_handle("cf1").unwrap();
	assert_eq!(secondary_db.get_cf(&secondary_cf1, b"k1").unwrap().unwrap(), b"v1");
	...
	assert_eq!(secondary_db.get_cf(&secondary_cf1, b"k1").unwrap().unwrap(), b"v2");
}
...
fn test_ttl_mix() {
	...
	let no_ttl = ColumnFamilyDescriptor::new_with_ttl("no_ttl", Options::default(), ColumnFamilyTtl::Disabled);

	let db = DB::open_cf_descriptors_with_ttl(&opts, &path, [cf1, no_ttl], Duration::from_secs(1)).unwrap();
	db.put(b"key1", b"value1").unwrap();
...
fn test_open_with_multiple_refs_as_single_threaded() {
	// This tests multiple references CANNOT be allowed while creating column
	// families
...
fn prefix_extract_and_iterate_test() {
	let path = DBPath::new("_rust_rocksdb_prefix_extract_and_iterate");
	{
		...
		assert_iter(
			db.iterator_opt(IteratorMode::Start, readopts),
			&[pair(b"p1_k1", b"v1"), pair(b"p1_k3", b"v3"), pair(b"p1_k4", b"v4")],
		);
	}
}
	...
	for mut batch in batches {
		...
		// bulk loading
		{
			...
			livefiles.iter().for_each(|f| {
				...
				assert!(!f.name.is_empty());
				assert_eq!(f.start_key.as_ref().unwrap().as_slice(), format!("{:0>4}", 0).as_bytes());
				assert_eq!(f.end_key.as_ref().unwrap().as_slice(), format!("{:0>4}", 9999).as_bytes());
			...
			assert!(
				db.delete_file_in_range(format!("{:0>4}", 0).as_bytes(), format!("{:0>4}", 9999).as_bytes())
					.is_ok()
			);
			let livefiles = db.live_files().unwrap();
...
fn test_open_cf_descriptors_for_read_only() {
	...
	}
	{
		...
		let cfs = cfs
			.into_iter()
			.map(|name| ColumnFamilyDescriptor::new(name, Options::default()));
		let db = DB::open_cf_descriptors_read_only(&opts, &path, cfs, error_if_log_file_exist).unwrap();
		let cf1 = db.cf_handle("cf1").unwrap();
		assert_eq!(db.get_cf(&cf1, b"k1").unwrap().unwrap(), b"v1");
		assert!(db.put_cf(&cf1, b"k2", b"v2").is_err());
	}
...
fn key_may_exist_cf_value() {
...

	{
		...
		for i in 0..10000i32 {
			let (may_exist, value) = db.key_may_exist_cf_opt_value(&cf, i.to_le_bytes(), &ReadOptions::default());
...
fn test_atomic_flush_cfs() {
...

	{
		...
		assert_eq!(db.get_cf(&cf1, "k11").unwrap(), Some("v11".as_bytes().to_vec()));
		assert_eq!(db.get_cf(&cf2, "k21").unwrap(), Some("v21".as_bytes().to_vec()));
	}
}
