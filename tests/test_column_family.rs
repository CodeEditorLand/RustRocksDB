...
fn test_column_family_with_transactiondb() {
	...
	// should fail to open db without specifying same column families
	{
		...
		match db {
			Ok(_db) => {
				panic!(
					"should not have opened TransactionDB successfully without specifying column
            families"
				)
			},
			Err(e) => assert!(e.to_string().starts_with("Invalid argument")),
	...
	// should properly open db when specifying all column families
	{
		...
		let cfs = &["cf1"];
		#[cfg(feature = "multi-threaded-cf")]
		let db = TransactionDB::<MultiThreaded>::open_cf(&opts, &TransactionDBOptions::default(), &n, cfs);
		#[cfg(not(feature = "multi-threaded-cf"))]
		let db = TransactionDB::<SingleThreaded>::open_cf(&opts, &TransactionDBOptions::default(), &n, cfs);
		match db {
			Ok(_db) => println!("successfully opened db with column family"),
			Err(e) => panic!("failed to open db with column family: {}", e),
		}
	...
	// should be able to drop a cf
	{
		...
		let cfs = &["cf1"];
		#[cfg(feature = "multi-threaded-cf")]
		let db = TransactionDB::<MultiThreaded>::open_cf(&opts, &TransactionDBOptions::default(), &n, cfs).unwrap();
		#[cfg(not(feature = "multi-threaded-cf"))]
		let mut db = TransactionDB::<SingleThreaded>::open_cf(&opts, &TransactionDBOptions::default(), &n, cfs).unwrap();
		match db.drop_cf("cf1") {
			Ok(_) => println!("cf1 successfully dropped."),
			Err(e) => panic!("failed to drop column family: {}", e),
		}
...
fn test_no_leaked_column_family() {
	let n = DBPath::new("_rust_rocksdb_no_leaked_column_family");
	{
		...
		// only if MultiThreaded, cf can outlive db.drop_cf() and shouldn't cause
		// SEGV...
		#[cfg(feature = "multi-threaded-cf")]
		{
