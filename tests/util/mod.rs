...
pub fn pair(left:&[u8], right:&[u8]) -> Pair { (Box::from(left), Box::from(right)) }
...
impl U64Timestamp {
	...
	pub fn new(ts:u64) -> Self { Self(ts.to_le_bytes()) }
}
...
impl From<U64Timestamp> for Vec<u8> {
	fn from(ts:U64Timestamp) -> Self { ts.0.to_vec() }
}
...
impl U64Comparator {
	...
	pub fn compare(a:&[u8], b:&[u8]) -> Ordering {
		...
		Self::compare_ts(extract_timestamp_from_user_key(a), extract_timestamp_from_user_key(b)).reverse()
	}
...
fn extract_timestamp_from_user_key(key:&[u8]) -> &[u8] { &key[(key.len() - U64Timestamp::SIZE)..] }

fn strip_timestamp_from_user_key(key:&[u8]) -> &[u8] { &key[..(key.len() - U64Timestamp::SIZE)] }
